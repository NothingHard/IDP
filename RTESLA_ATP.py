...
W1 = np.random.uniform(high=0.1,low=-0.1,size=(784,hidden)).astype('float32')
b1 = np.random.uniform(high=0.1,low=-0.1,size=(hidden)).astype('float32')
W2 = np.random.uniform(high=0.1,low=-0.1,size=(hidden,hidden)).astype('float32')
b2 = np.random.uniform(high=0.1,low=-0.1,size=(hidden)).astype('float32')   
W3 = np.random.uniform(high=0.1,low=-0.1,size=(hidden,10)).astype('float32')
...
with tf.Session() as sess:
    with tf.variable_scope(profile+str(counter)) as scope:
        ...
        r2_dict = {}
        log = pd.DataFrame()
        save_dir = "~/IDP/output_random_new_ATP/"
        epochs = 1
        batch_per_epoch = 200
        batch_size = 28
        learning_rate = 0.001
        ...
        while go_on:
            ...
            if alternate and trained_idp[tidp]==1.0:
                intv += 1
